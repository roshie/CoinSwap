...
INITIAL_PRICE_FEED_VALUE = 200000000000000000000
...
def get_mood(number):
    switch = {0: "CUTE", 1: "STARE", 2: "ANGRY"}
    return switch[number]

def get_color(number):
    switch = {0: "White", 1: "Grey", 2: "Brown"}
    return switch[number]
...
def deploy_mocks(decimals=18, initial_value=INITIAL_PRICE_FEED_VALUE):
    ...
    print("Deploying Mock DAI and WETH")
    dai_token = MockDAI.deploy({"from": account})
    weth_token = MockWETH.deploy({"from": account})
    print(f"MockDAI Address: {dai_token.address}")
    print(f"MockWETH Address: {weth_token.address}")